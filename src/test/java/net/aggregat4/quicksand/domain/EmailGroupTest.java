...
public class EmailGroupTest {
  ...
  private static final Clock FIXED_CLOCK =
      Clock.fixed(Instant.parse("2026-03-25T09:15:00Z"), ZoneId.of("Europe/Berlin"));
  ...
  void groupsDescendingHeadersAcrossAllTemporalBuckets() {
    ...
    List<EmailGroup> groups =
        EmailGroup.createEmailGroups(headers, FIXED_CLOCK, SortOrder.DESCENDING);

    assertEquals(
        List.of("Today", "This Week", "Last Week", "This Month", "Last Three Months", "Older"),
        groups.stream().map(group -> group.name().orElseThrow()).toList());
    assertEquals(
        List.of(2, 2, 2, 2, 2, 1), groups.stream().map(EmailGroup::getNofMessages).toList());
  }
  ...
  void groupsAscendingHeadersAcrossAllTemporalBuckets() {
    ...
    List<EmailGroup> groups =
        EmailGroup.createEmailGroups(headers, FIXED_CLOCK, SortOrder.ASCENDING);

    assertEquals(
        List.of("Older", "Last Three Months", "This Month", "Last Week", "This Week", "Today"),
        groups.stream().map(group -> group.name().orElseThrow()).toList());
    assertEquals(
        List.of(1, 2, 2, 2, 2, 2), groups.stream().map(EmailGroup::getNofMessages).toList());
  }

  private static List<EmailHeader> boundaryHeadersDescending() {
    ZonedDateTime todayStart =
        ZonedDateTime.now(FIXED_CLOCK).toLocalDate().atStartOfDay(FIXED_CLOCK.getZone());
    ZonedDateTime weekStart =
        GroupedPeriod.getBeginningOfWeek(FIXED_CLOCK).atStartOfDay(FIXED_CLOCK.getZone());
    ZonedDateTime lastWeekStart = weekStart.minusWeeks(1);
    ZonedDateTime monthStart =
        GroupedPeriod.getBeginningOfMonth(FIXED_CLOCK).atStartOfDay(FIXED_CLOCK.getZone());
