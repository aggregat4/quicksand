...
public class SqliteTest {
  ...
  public void createDatabaseFtsIndexAndQuery() throws SQLException, IOException {
    ...
    try (Connection connection = ds.getConnection();
      ...
      //        statement.executeUpdate("create table emails (id INTEGER PRIMARY KEY, subject
      // string, body string)");
      statement.executeUpdate("CREATE VIRTUAL TABLE emailfts USING fts5(subject, body)");
      statement.executeUpdate(
          "insert into emailfts values('I am the son of a prince of nigeria', 'I was wondering if you are interested in a proposal.')");
      statement.executeUpdate("insert into emailfts values('yo son', 'wassup? you are the man')");
      try (ResultSet rs =
          statement.executeQuery(
              "SELECT highlight(emailfts, 0, '<b>', '</b>'), highlight(emailfts, 1, '<b>', '</b>') FROM emailfts WHERE emailfts MATCH 'yo' ORDER BY rank")) {
        while (rs.next()) {
