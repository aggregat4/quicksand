...
public class SchemaMigrator {
  ...
  private static final Function<Connection, Integer> v1Migration =
      (con) -> {
    ...
    try (var stmt = con.prepareStatement("UPDATE schema_version SET version = ? WHERE id = ?")) {
      ...
      if (affectedRows != 1) {
        throw new IllegalStateException(
            "Database schema_version table could not be updated correctly, %s rows were affected in update statement"
  ...
  private static int determineSchemaVersion(Connection con) {
    if (tableExists(con, "schema_version")) {
      return withPreparedStmtFunction(
          con,
  ...
  public static void migrate(Connection con, Migrations migrations) {
    ...
    if (dbVersion > migrations.getCurrentVersion()) {
      throw new IllegalStateException(
          "Database version is higher than code version: %s".formatted(dbVersion));
    }
    if (dbVersion < 0) {
    ...
    for (int i = startVersion; i <= migrations.getCurrentVersion(); i++) {
      ...
      if (migration == null) {
        throw new IllegalStateException(
            "Do not have a migration for version %s of the schema".formatted(i));
      }
