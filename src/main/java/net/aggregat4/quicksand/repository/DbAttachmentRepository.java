...
public class DbAttachmentRepository implements AttachmentRepository {
  ...
  @Override
  public Attachment saveDraftAttachment(
      int draftId,
      String name,
      long sizeInBytes,
  ...
  public Optional<StoredAttachment> findStoredById(int id) {
    return DbUtil.withPreparedStmtFunction(
        ...
        stmt -> {
          stmt.setInt(1, id);
          return DbUtil.withResultSetFunction(
              stmt, rs -> rs.next() ? Optional.of(toStoredAttachment(rs)) : Optional.empty());
        });
  }

  @Override
  public void moveDraftAttachmentsToOutboundMessage(
      Connection con, int draftId, int outboundMessageId) {
