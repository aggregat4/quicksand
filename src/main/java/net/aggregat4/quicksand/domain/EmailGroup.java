...
public record EmailGroup(List<EmailHeader> headers, GroupedPeriod period) {
  ...
  public static List<EmailGroup> createEmailGroups(
      List<EmailHeader> emailHeaders, Clock clock, SortOrder sortOrder) {
        ...
        if (currentGroupedPeriod.matches(emailHeader, clock)) {
          ...
          if (currentPeriodIndex == groupedPeriods.length - 1) {
            throw new IllegalStateException(
                "We have reached the end of the available grouped periods but no groups matches this particular email, this should never happen. EmailHeader: %s"
                    .formatted(emailHeader));
          }
