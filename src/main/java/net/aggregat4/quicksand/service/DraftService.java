...
public class DraftService {
  ...
  public Optional<Draft> saveDraft(
      int id, String to, String cc, String bcc, String subject, String body) {
  ...
  private static String replyBody(Email email) {
    ...
    if (sourceBody.isBlank()) {
      return "\n\nOn %s, %s wrote:\n"
          .formatted(email.header().longFormattedSentDate(), email.header().getSender());
    }
    return "\n\nOn %s, %s wrote:\n%s"
        .formatted(
  ...
  private static String forwardBody(Email email) {
    String recipients =
        email.header().getRecipients().stream()
            .map(Actor::toString)
