...
public class GreenmailUtils {
    private record DemoMessageTemplate(String subject, String from, String plainTextBody, String htmlBody) { }
    ...
    private static List<DemoMessageSeed> buildDemoSeeds(int count, Clock clock) {
        ...
        for (int i = 0; i < fillerCount; i++) {
            ...
            ZonedDateTime receivedAt = oldestBoundaryTimestamp
                    .minusHours(6)
                    .minusHours(12L * clusterIndex);
    ...
    private static List<DemoMessageSeed> buildBoundarySeeds(Clock clock) {
        ...
        ZonedDateTime monthStart = GroupedPeriod.getBeginningOfMonth(clock).atStartOfDay(clock.getZone());
        ZonedDateTime lastThreeMonthsStart = monthStart.minusMonths(2);

        return List.of(
                seed("Today latest - welcome to the demo inbox", "hello@quicksand.demo", """
                ...
                seed("HTML demo: Product launch digest", "launches@example.com", """
                        Launch Digest
                ...
                seed("Today exact - start of day", "boundaries@example.com", """
                        This message sits exactly at the start of the current day.
                        """, null, todayStart),
                seed("This week boundary - one minute before today", "boundaries@example.com", """
                        This message should group as This Week.
                        """, null, todayStart.minusMinutes(1)),
                seed("This week exact - start of week", "boundaries@example.com", """
                        This message sits exactly on the start-of-week boundary.
                        """, null, weekStart),
                seed("Last week boundary - one minute before this week", "boundaries@example.com", """
                        This message should group as Last Week.
                        """, null, weekStart.minusMinutes(1)),
                seed("Last week exact - start of last week", "boundaries@example.com", """
                        This message sits exactly on the start of last week.
                        """, null, lastWeekStart),
                seed("This month boundary - one minute before last week", "boundaries@example.com", """
                        This message should group as This Month.
                        """, null, lastWeekStart.minusMinutes(1)),
                seed("This month exact - start of month", "boundaries@example.com", """
                        This message sits exactly on the start of the current month.
                        """, null, monthStart),
                seed("Last three months boundary - one minute before this month", "boundaries@example.com", """
                        This message should group as Last Three Months.
                        """, null, monthStart.minusMinutes(1)),
                seed("Last three months exact - start of window", "boundaries@example.com", """
                        This message sits exactly on the start of the last-three-month window.
                        """, null, lastThreeMonthsStart),
                seed("Older boundary - one minute before last three months", "boundaries@example.com", """
                        This message should group as Older.
                        """, null, lastThreeMonthsStart.minusMinutes(1)),
                seed("HTML demo: Quarterly update", "news@example.com", """
                        Quarterly update
    ...
    private static DemoMessageSeed seed(String subject, String from, String plainTextBody, String htmlBody, ZonedDateTime receivedAt) {
        return new DemoMessageSeed(new DemoMessageTemplate(subject, from, plainTextBody, htmlBody), receivedAt);
    }
    ...
    private static MimeMessage createDemoMessage(GreenMailOperations greenMail, DemoMessageTemplate template, ZonedDateTime sentAt) {
        try {
    ...
    private static void appendMessage(MailFolder inbox, MimeMessage message, ZonedDateTime receivedAt) {
