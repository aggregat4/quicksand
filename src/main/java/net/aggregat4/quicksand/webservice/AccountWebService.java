...
public class AccountWebService implements HttpService {
    ...
    private void getSearchHandler(ServerRequest request, ServerResponse response) {
        ...
        EmailPage emailPage = emailService.searchMessages(
                accountId,
                query.get(),
                effectivePageSize,
                offsetReceivedTimestamp.orElse(defaultOffsetReceivedTimestamp(pageParams)),
                ...
                pageParams.pageDirection(),
                pageParams.sortOrder());
        Pagination pagination = new Pagination(offsetReceivedTimestamp, offsetMessageId, pageParams, effectivePageSize, Optional.of(resultCount), emailPage.hasLeft(), emailPage.hasRight());
