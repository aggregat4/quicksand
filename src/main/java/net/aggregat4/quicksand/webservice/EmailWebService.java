...
public class EmailWebService implements HttpService {
  ...
  public void routing(HttpRules rules) {
    // NOTE: using a subresource to distinguish between a viewer and a composer view type. This
  ...
  private void emailActionHandler(ServerRequest request, ServerResponse response) {
    ...
    // Mailbox actions update local state and enqueue the corresponding background IMAP operation.
    switch (action) {
    ...
    while (multiPart.hasNext()) {
      ...
      try (var inputStream = part.inputStream()) {
        attachmentService.storeDraftAttachment(
            emailId, fileName.get(), part.contentType(), inputStream);
      } catch (IOException e) {
  ...
  private void emailSendOrDeleteHandler(ServerRequest request, ServerResponse response) {
    ...
    if ((!shouldDeleteEmail && !shouldSendEmail) || (shouldDeleteEmail && shouldSendEmail)) {
      throw new BadRequestException(
          "Posting to a concrete email requires either the delete or send action");
    }
    if (shouldDeleteEmail) {
      ...
      ResponseUtils.redirectAfterPost(
          response, URI.create("/emails/%s/queued?result=deleted".formatted(emailId)));
      return;
    }
    ...
    if (!validationErrors.isEmpty()) {
      ...
      ResponseUtils.redirectAfterPost(
          response,
          URI.create("/emails/%s/queued?result=queued".formatted(queuedMessage.get().id())));
    }
  }
  ...
  private static void redirectWithValidationErrors(int emailId, String s, ServerResponse response) {
    String relativeUrl =
        "/emails/%s/composer?validationErrors=%s"
            .formatted(emailId, URLEncoder.encode(s, StandardCharsets.UTF_8));
